...
class ActivityMetaData:
    ...
    def __post_init__(self):
        ...
        if self.mean_kmph is None:
            self.mean_kmph = 3600 / self.mean_km_pace.seconds
            #print(f'kmph_mean {self.kmph_mean}')
        if self.mean_mph is None:
            self.mean_mph = self.mean_kmph / MILE
        if self.mean_mile_pace is None:
            self.mean_mile_pace = pd.to_timedelta((60 / self.mean_mph), unit='s')
        if self.day is None:
...
class Activity:
    ...
    def __init__(self, config: Config, points: pd.DataFrame, laps: Optional[pd.DataFrame] = None,
        ...
        if metadata is not None:
            ...
            if kwargs.get('mean_km_pace') is None:
                kwargs['mean_km_pace'] = self.points['km_pace'].mean()
