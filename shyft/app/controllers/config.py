...
class ConfigController(_BaseDashController):
    ...
    def generate_form(self) -> dbc.Form:
        data_dir_input = dbc.FormGroup([
            ...
            dbc.Col(dbc.Input(id='data_dir', type='text', value=self.config.data_dir), width=self.input_width),
        ], row=True)
        name_input = dbc.FormGroup([
            dbc.Label('Your name', html_for='user_name', width=self.label_width),
            dbc.Col(dbc.Input(id='user_name', type='text', value=self.config.user_name), width=self.input_width)
        ], row=True)
        dist_unit_input = dbc.FormGroup([
            ...
            ], value=self.config.distance_unit), width=self.input_width)
        ], row=True)
        match_center_input = dbc.FormGroup([
            ...
            dbc.Col(dbc.Input(id='match_center', type='number', value=self.config.match_center_threshold),
        ...
        match_length_input = dbc.FormGroup([
            ...
            dbc.Col(dbc.Input(id='match_length', type='number', value=self.config.match_length_threshold),
                    width=self.input_width)
        ], row=True)
        tight_match_input = dbc.FormGroup([
            dbc.Label('Threshold for tight-matching routes', html_for='tight_match', width=self.label_width),
            dbc.Col(dbc.Input(id='tight_match', type='number', value=self.config.tight_match_threshold),
                    width=self.input_width)
        ], row=True)
        name_fmt_input = dbc.FormGroup([
            dbc.Label('Format for displaying activity name (where no activity name is specified)',
                      width=self.label_width),
            dbc.Col(dbc.Textarea(id='name_fmt', value=self.config.default_activity_name_format),
                    width=self.input_width)
        ], row=True)
        week_start_input = dbc.FormGroup([
            ...
            dbc.Col(dcc.Dropdown(id='week_start', options=[{'label': d, 'value': d} for d in DAYS_OF_WEEK],
                                 value=self.config.week_start), width=self.input_width)
        ], row=True)
    ...
    def register_callbacks(self):
        ...
        def save_config(save_button_clicks: int, close_button_clicks: int, data_dir: str, user_name: str,
            ...
            if button_id == 'save_button':
                ...
                self.config.match_center_threshold = match_center
                self.config.match_length_threshold = match_length
                self.config.tight_match_threshold = tight_match
                self.config.default_activity_name_format = name_fmt
